...
SELECT
    ...
    p.occupied_place_type,
    UPPER(p.center_name) as center_name,
    SUM(p.total_users) AS total_users,
    ROUND(AVG(p.average_age)::numeric, 2) AS avg_age,
    ROUND(STDDEV(p.average_age)::numeric, 2) AS stddev_age,
    ROUND(AVG(p.average_height)::numeric, 2) AS avg_height,
    ROUND(STDDEV(p.average_height)::numeric, 2) AS stddev_height,
    ROUND(AVG(p.average_weight)::numeric, 2) AS avg_weight,
    ROUND(STDDEV(p.average_weight)::numeric, 2) AS stddev_weight
FROM
...
JOIN 
    public.dim_centers dc ON p.center_identifier = dc.center_identifier
WHERE
    UPPER(dc.center_name) = 'ALDAY'  -- Filtra por el nombre de centro "ALDAY"
GROUP BY 
    dc.center_identifier, 
    dc.center_name,
    dc.manager_legal_personality,
...
select * from catalog_statistics_centers_people
...
FROM
    catalog_statistics_centers_people p
WHERE
    p.manager_legal_personality LIKE '%PUBLICA%'  -- Filtra por la palabra "publica"
GROUP BY
    p.occupied_place_type;

CREATE VIEW view_titularidad AS
SELECT
    p.manager_legal_personality,
    SUM(p.total_users) AS total_users,
    ROUND(AVG(p.average_age)::numeric, 2) AS avg_age,
